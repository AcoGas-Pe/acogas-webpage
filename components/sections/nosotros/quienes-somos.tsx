...
export function QuienesSomos() {
  return (
    <section id="quienes-somos" className="section mx-auto bg-background text-foreground text-center">
          ...
          <div className="flex sm:flex-row flex-col items-start text-left justify-start p-4 gap-8 text-foreground">
            <div className="flex flex-col items-start justify-start gap-4 w-full sm:w-1/2">
              <p className="text-base sm:text-lg text-foreground/80 leading-relaxed">
                ...
                es un socio estratégico de la industria, especializado en el diseño, provisión y acompañamiento
                técnico-comercial de soluciones integrales para GLP, gas natural, vapor y otros procesos industriales,
                mediante equipos, ingeniería aplicada y servicios especializados.
              </p>
              <p className="text-base sm:text-lg text-foreground/80 leading-relaxed">
                Comercializamos y representamos marcas de referencia mundial —entre ellas Fisher (GLP, GN y otros),
                Tartarini, Spence, Cash, Anderson Greenwood, Crosby, Kunkle, Marston, Enardo, Varec, Corken, Liquid
                Controls y Cavagna— con el fin de asegurar seguridad, eficiencia operativa, precisión, cumplimiento
                normativo y confiabilidad en cada proyecto, desde la selección del equipo hasta su aplicación en campo.
              </p>
              <div className="flex flex-col items-start gap-3 p-4 rounded-lg border border-primary/15 bg-primary/5 w-full">
                ...
                <p className="text-sm text-foreground/70 leading-relaxed">{CENTRAL_MESSAGE_ALT}</p>
                <p className="text-xs sm:text-sm text-muted-foreground italic border-l-2 border-accent pl-3">
                  {TECH_COMMERCIAL_SLOGAN}
                </p>
                <p className="text-xs text-muted-foreground/80 italic">
                  Frase de misión: <span className="font-medium not-italic text-foreground/80">{MISSION_TAGLINE}</span>
                </p>
              </div>
            </div>
            <Image
